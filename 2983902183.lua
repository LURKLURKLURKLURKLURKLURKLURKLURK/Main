...
do
    ...
    function library:New(info)
        ...
        function window:Watermark(info)
            ...
            local watermark_title = utility:Create("TextLabel", {Vector2.new(2 + 6,4), watermark_outline}, {
                Text = string.format("pie spie - fps : %u - uid : %u", 35, 2),
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
                OutlineColor = theme.textborder,
    ...
    --
    function pages:AddSection(info)
        local info = info or {}
    ...
    --
    function pages:AddMultiSection(info)
        local info = info or {}
    ...
    function sections:Label(info)
        ...
        local flag = info.flag or info.flag or info.flag or info.Flag or nil
        --
        local window = self.window
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = label
        end
        --
    ...
    function sections:Textbox(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = textbox
        end
        --
    ...
    function sections:AddToggle(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = toggle
        end
        --
        section.currentAxis = section.currentAxis + 15 + 4
        section:Update()
        --
        function toggle:AddColor(info)
            local info = info or {}
            ...
            --
            if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
                library.flags[tostring(flag)] = colorpicker
            end
            --
        ...
        function toggle:Keybind(info)
            ...
            --
            if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
                library.flags[tostring(flag)] = keybind
            end
            --
            keybind.active = keybind.mode == "Always" and true or false
            --
    ...
    function sections:AddSlider(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = slider
        end
        --
    ...
    function sections:Button(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = button
        end
        --
    ...
    --
    function sections:AddList(info)
        local info = info or {}
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = dropdown
        end
        --
    ...
    function sections:Multibox(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = multibox
        end
        --
        section.currentAxis = section.currentAxis + 35 + 4
        section:Update()
    ...
    --
    function sections:AddColor(info)
        local info = info or {}
        ...
        section:Update()
        --
        function colorpicker:AddColor(info)
            local info = info or {}
            local cpinfo = info.info or info.Info or name
            local def = info.def or info.Def or info.default or info.Default or Color3.fromRGB(255, 0, 0)
            local transp = info.transparency or info.Transparency or info.transp or info.Transp or info.alpha or info.Alpha or nil
    ...
    function sections:Listbox(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = listbox
        end
        --
    ...
    function sections:ConfigBox(info)
        ...
        --
        if flag and tostring(flag) ~= "" and tostring(flag) ~= " " and not library.flags[tostring(flag)] then
            library.flags[tostring(flag)] = configLoader
        end
        --
        section.currentAxis = section.currentAxis + 148 + 4
        section:Update()
...
--[[do
    ...
    --
    local test = lib:AddTab({name = "test"})
    local sucky = lib:AddTab({name = "sucky"})

    local testMain = test:AddSection({name = "Main"})
...
return library, library.flags, theme -- utility
